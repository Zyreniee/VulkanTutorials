...
class Engine:
    ...
    def makeAssets(self, _scene: scene.Scene) -> None:
        ...
        sphere_count = int(len(_scene.spheres) / 8)
        self.sphereBuffer = buffer.Buffer(size = sphere_count, binding = 1, floatCount = 8, dtype=np.float32)
        self.nodeBuffer = buffer.Buffer(size = _scene.nodes_used, binding = 2, floatCount = 8, dtype=np.float32)
        self.indexBuffer = buffer.Buffer(size = len(_scene.sphere_ids), binding = 3, floatCount = 1, dtype=np.int32)
    ...
    def renderScene(self, scene: scene.Scene):
        ...
        #glDispatchCompute(int(self.colorBuffer.size[0] / 8), int(self.colorBuffer.size[1] / 8), 1)
