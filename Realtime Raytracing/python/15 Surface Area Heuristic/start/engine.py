...
class Engine:
    ...
    def makeAssets(self) -> None:
        ...
        self.sphereBuffer = buffer.Buffer(size = 3000, binding = 1, floatCount = 8, dtype=np.float32)
        self.nodeBuffer = buffer.Buffer(size = 6000, binding = 2, floatCount = 8, dtype=np.float32)
        self.indexBuffer = buffer.Buffer(size = 3000, binding = 3, floatCount = 1, dtype=np.int32)
