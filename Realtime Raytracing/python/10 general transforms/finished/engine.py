...
class Engine:
    ...
    def recordSphere(self, i, _sphere):
        ...
        model_transform = pyrr.matrix44.multiply(
            m1 = model_transform,
            m2 = pyrr.matrix44.create_from_scale(scale = _sphere.radii, dtype = np.float32)
        )
        model_transform = pyrr.matrix44.multiply(
            m1 = model_transform,
            m2 = pyrr.matrix44.create_from_quaternion(
                quat= pyrr.quaternion.create_from_eulers(
                    _sphere.eulers,
                    dtype=np.float32
                ), 
                dtype = np.float32
        ...
        model_transform = pyrr.matrix44.multiply(
            m1 = model_transform,
            m2 = pyrr.matrix44.create_from_translation(vec = _sphere.center, dtype = np.float32)
        )

        inverse_model_transform = np.reshape(np.linalg.inv(model_transform), 16)

        # sphere: (col1) (col2) (col3) (col3) (r g b roughness)

        self.objectData[20*i : 20*i + 16] = inverse_model_transform[:]

        self.objectData[20*i + 16 : 20*i + 19] = _sphere.color[0]

        self.objectData[20*i + 20] = _sphere.roughness
    
    def recordPlane(self, i, _plane):

        # plane: (cx cy cz tx) (ty tz bx by) (bz nx ny nz) (umin umax vmin vmax) (r g b -)

        self.objectData[20*i : 20*i + 3] = _plane.center[:]

        self.objectData[20*i + 3 : 20*i + 6] = _plane.tangent[:]

        self.objectData[20*i + 6 : 20*i + 9] = _plane.bitangent[:]

        self.objectData[20*i + 9 : 20*i + 12]  = _plane.normal[:]
    ...
    def recordLight(self, i, _light):
        ...
        self.objectData[20*i : 20*i + 3] = _light.position[:]

        self.objectData[20*i + 3] = _light.strength

        self.objectData[20*i + 4 : 20*i + 7] = _light.color[:]
    ...
    def updateScene(self, scene):
        ...
        glUniform1f(glGetUniformLocation(self.rayTracerShader, "lightCount"), len(scene.lights))

        for i,_light in enumerate(scene.lights):
            self.recordLight(i + len(scene.spheres) + len(scene.planes), _light)
