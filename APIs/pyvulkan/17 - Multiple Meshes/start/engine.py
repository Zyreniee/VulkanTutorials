...
class Engine:
    ...
    def make_assets(self):
        ...
        self.square_mesh = square_mesh.SquareMesh(self.device, self.physicalDevice)
        self.star_mesh = star_mesh.StarMesh(self.device, self.physicalDevice)
    ...
    def record_draw_commands(self, commandBuffer, imageIndex, scene):
        ...
        vkCmdBindVertexBuffers(
            commandBuffer = commandBuffer, firstBinding = 0,
            bindingCount = 1, pBuffers = (self.square_mesh.vertex_buffer.buffer,),
            pOffsets = (0,)
        )
        for position in scene.square_positions:
            ...
            vkCmdDraw(
                commandBuffer = commandBuffer, vertexCount = 6, 
                instanceCount = 1, firstVertex = 0, firstInstance = 0
            )
        
        vkCmdBindVertexBuffers(
            commandBuffer = commandBuffer, firstBinding = 0,
            bindingCount = 1, pBuffers = (self.star_mesh.vertex_buffer.buffer,),
            pOffsets = (0,)
        )
        for position in scene.star_positions:
            ...
            vkCmdDraw(
                commandBuffer = commandBuffer, vertexCount = 24, 
                instanceCount = 1, firstVertex = 0, firstInstance = 0
            )
