...
Engine::Engine(GLFWwindow* window) :
	...
	descriptorSetLayout = descriptorSetLayoutBuilder.build(deviceDeletionQueue);

	PipelineLayoutBuilder pipelineLayoutBuilder(logicalDevice);
	pipelineLayoutBuilder.add(descriptorSetLayout);
	pipelineLayout = pipelineLayoutBuilder.build(deviceDeletionQueue);
	
	shader = make_compute_shader(logicalDevice, "clear_screen", dldi, 
		deviceDeletionQueue, &descriptorSetLayout);
	...
	descriptorPool = make_descriptor_pool(logicalDevice, 2, descriptorTypes.size(), 
	...
	for (uint32_t i = 0; i < 2; ++i) {
		...
		descriptorSets[i] = allocate_descriptor_set(logicalDevice, descriptorPool, descriptorSetLayout);
		frames.push_back(Frame(swapchain, logicalDevice, shader, dldi, commandBuffer, 
			deviceDeletionQueue, descriptorSets[i], pipelineLayout));
	}
