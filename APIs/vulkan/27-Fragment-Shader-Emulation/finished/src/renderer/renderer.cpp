...
Engine::Engine(GLFWwindow* window) :
	...
	descriptorSetLayouts[DescriptorScope::eFrame] = descriptorSetLayoutBuilder.build(deviceDeletionQueue);
	descriptorSetLayoutBuilder.add_entry(vk::ShaderStageFlagBits::eCompute, vk::DescriptorType::eStorageBuffer);
	descriptorSetLayouts[DescriptorScope::eDrawCall] = descriptorSetLayoutBuilder.build(deviceDeletionQueue);

	PipelineLayoutBuilder pipelineLayoutBuilder(logicalDevice);
	pipelineLayoutBuilder.add(descriptorSetLayouts[DescriptorScope::eFrame]);
	pipelineLayouts[PipelineType::eClear] = pipelineLayoutBuilder.build(deviceDeletionQueue);
	pipelineLayoutBuilder.add(descriptorSetLayouts[DescriptorScope::eFrame]);
	pipelineLayoutBuilder.add(descriptorSetLayouts[DescriptorScope::eDrawCall]);
	pipelineLayouts[PipelineType::eRasterizeSmall] = pipelineLayoutBuilder.build(deviceDeletionQueue);
	pipelineLayoutBuilder.add(descriptorSetLayouts[DescriptorScope::eFrame]);
	pipelineLayoutBuilder.add(descriptorSetLayouts[DescriptorScope::eDrawCall]);
	pipelineLayouts[PipelineType::eRasterizeBig] = pipelineLayoutBuilder.build(deviceDeletionQueue);
	...
	shaders[PipelineType::eRasterizeSmall] = make_compute_shader(logicalDevice, "rasterize", dldi,
		deviceDeletionQueue, layouts);
	shaders[PipelineType::eRasterizeBig] = make_compute_shader(logicalDevice, "rasterize_big_triangle", dldi,
		deviceDeletionQueue, layouts);
	...
	for (uint32_t i = 0; i < 2; ++i) {
		...
		descriptorSets[i][DescriptorScope::eFrame] = allocate_descriptor_set(logicalDevice, 
			descriptorPools[DescriptorScope::eFrame], 
			descriptorSetLayouts[DescriptorScope::eFrame]);
		descriptorSets[i][DescriptorScope::eDrawCall] = allocate_descriptor_set(logicalDevice, 
			descriptorPools[DescriptorScope::eDrawCall], 
			descriptorSetLayouts[DescriptorScope::eDrawCall]);
