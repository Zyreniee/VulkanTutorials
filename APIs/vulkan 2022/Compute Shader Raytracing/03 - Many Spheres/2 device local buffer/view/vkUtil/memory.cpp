...
void vkUtil::allocateBufferMemory(vkUtil::BufferOutputChunk& buffer, const BufferInputChunk& input) {
	...
	*/
	...
	buffer.memory = input.logicalDevice.allocateMemory(allocInfo);
	input.logicalDevice.bindBufferMemory(buffer.buffer, buffer.memory, 0);
}

vkUtil::BufferOutputChunk vkUtil::createBuffer(BufferInputChunk input) {
	...
	*/
	...
	vkUtil::BufferOutputChunk buffer = {};
...
void vkUtil::copyBuffer(vk::Buffer& srcBuffer, vk::Buffer& dstBuffer, vk::DeviceSize size, vk::Queue queue, vk::CommandBuffer commandBuffer) {
