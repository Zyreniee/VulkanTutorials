...
namespace vkInit {
	...
	*/
	SwapChainBundle create_swapchain(vk::Device logicalDevice, vk::PhysicalDevice physicalDevice, vk::SurfaceKHR surface, int width, int height, Scene* scene) {
	  ...
	  VULKAN_HPP_NAMESPACE::SwapchainKHR   oldSwapchain_ = {} ) VULKAN_HPP_NOEXCEPT
		*/
		...
		for (size_t i = 0; i < images.size(); ++i) {
			...
			bundle.frames.push_back(vkUtil::SwapChainFrame(physicalDevice, logicalDevice, extent.width, extent.height, scene->spheres.size() * 32));
