...
void Engine::make_frame_resources() {
	...
	for (vkUtil::SwapChainFrame& frame : swapchainFrames) {
		...
		frame.make_descriptor_resources(device, physicalDevice, 64 * 32);
...
void Engine::prepare_frame(uint32_t imageIndex, Scene* scene) {
	...
	_frame.sphereBuffer.blit((void*)scene->spheres.data(), scene->spheres.size() * sizeof(scene->spheres[0]));
	_frame.sceneBuffer.blit((void*)&scene->description, 64);
