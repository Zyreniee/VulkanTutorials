...
std::vector<float> util::load_model_from_file(MeshCreateInfo* createInfo) {
...
std::vector<float> util::load_toon_model_from_file(MeshCreateInfo* createInfo) {
    ...
    while (std::getline(obj_file, line)) {
        ...
        if (splitLine[0].compare("mtllib") == 0) {
            ...
            std::vector<float> face = build_face(splitLine, v, color, vn, position_offset, normal_offset);
            for (float vertex_component : face) {
                vertices.push_back(vertex_component);
            }
        }
    }
    return vertices;
...
std::vector<float> util::build_face(
    ...
    */
    ...
    for (int i : vertex_order) {
        //position
        face.push_back(faceVertices[i].x);
        face.push_back(faceVertices[i].y);
        face.push_back(faceVertices[i].z);
        
        //color
        face.push_back(color.r);
        face.push_back(color.g);
        face.push_back(color.b);
            
        //normal
        face.push_back(faceNormals[i].x);
        face.push_back(faceNormals[i].y);
        face.push_back(faceNormals[i].z);
    }
