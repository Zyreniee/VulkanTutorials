...
Engine::Engine(int width, int height) {
	...
	frameBufferA = new FrameBuffer();
	frameBufferA->build(width, height);

	frameBufferB = new FrameBuffer();
	frameBufferB->build(width, height);
...
void Engine::createShaders() {
	...
	filepaths.fragment = "shaders/horizontal_blur_fragment.txt";
	horizontalBlurShader = util::load_shader(filepaths);

	filepaths.fragment = "shaders/vertical_blur_fragment.txt";
	verticalBlurShader = util::load_shader(filepaths);
...
void Engine::render(Scene* scene) {
	...
	frameBufferB->draw_to();
	frameBufferA->read_from();
	glDrawArrays(GL_TRIANGLES, 0, 6);

	glUseProgram(horizontalBlurShader);
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	frameBufferB->read_from();
	glDrawArrays(GL_TRIANGLES, 0, 6);
