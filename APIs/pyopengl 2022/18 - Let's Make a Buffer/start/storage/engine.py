...
class Engine:
    ...
    def makeAssets(self, _scene: scene.Scene) -> None:
        ...
        self.sphereBuffer = buffer.Buffer(size = sphere_count, binding = 1, dtype=DATA_TYPE_SPHERE)
        self.nodeBuffer = buffer.Buffer(size = _scene.nodes_used, binding = 2, dtype=DATA_TYPE_NODE)
        self.indexBuffer = buffer.Buffer(size = len(_scene.sphere_ids), binding = 3, dtype=np.int32)
