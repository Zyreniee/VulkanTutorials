...
class Reflector:
    ...
    def __init__(self, position, eulers, mesh_type):
...
class GraphicsEngine:
    ...
    def render_scene_objects(self, scene, mirror = None):
        ...
        if mirror is None:
            ...
            viewTransform = mirror.viewTransform
            viewPosition = mirror.position
        ...
        if mirror is not None:
            self.materials[scene.player.material_type].use()
            glUniformMatrix4fv(self.modelMatrixLocation["lit"],1,GL_FALSE,scene.player.modelTransform)
            glDrawArrays(
                GL_TRIANGLES, 
                self.offsets[FULL_ATTRIBUTES][scene.player.mesh_type][0], 
                self.offsets[FULL_ATTRIBUTES][scene.player.mesh_type][1]
            )
        ...
        if mirror is None:
            ...
            for mirror in scene.mirrors:
                ...
                glDrawArrays(
                    GL_TRIANGLES, 
                    self.offsets[FULL_ATTRIBUTES][MIRROR_MESH][0], 
                    self.offsets[FULL_ATTRIBUTES][MIRROR_MESH][1]
                )
    ...
    def render(self, scene):
        ...
        for mirror in scene.mirrors:
            self.render_scene_objects(scene, mirror)
        
        self.render_scene_objects(scene, None)
