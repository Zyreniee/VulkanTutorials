...
class App:
    ...
    def handle_keys(self) -> None:
        """
            React to the current key state.
        """
        ...
        if combo > 0:
            ...
            transform_component: TransformComponent = self.scene.player.transform
            eulers = transform_component.eulers
            
            dPos = [
                self.frame_time * 0.025 * np.cos(eulers[2] + np.deg2rad(direction_modifier)),
                self.frame_time * 0.025 * np.sin(eulers[2] + np.deg2rad(direction_modifier)),
                0
            ]
    ...
    def calculate_framerate(self) -> None:
        """
            must be called once per frame. Updates/Calculates the app's
            framerate.
        """
    ...
    def quit(self) -> None:
        """
            Destroy any allocated (Non-GC) resources.
        """
