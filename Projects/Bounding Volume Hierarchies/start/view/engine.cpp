...
glm::vec3 Engine::ray_color(Ray& ray, Scene* scene, int depth) {
	...
	if (hit_something) {
		Ray newRay;
		newRay.origin = rec.point;
		newRay.direction = glm::normalize(rec.normal + 0.9f * random_in_unit_sphere());
	...
	for (int y = 0; y < height; ++y) {
		...
		for (int x = 0; x < width; ++x) {
			...
			for (int sample = 0; sample < sampleCount; ++sample) {
				...
				Ray ray;
				ray.origin = scene->player->position;
				ray.direction = glm::normalize(scene->player->forwards + horizontalCoefficient * scene->player->right + verticalCoefficient * scene->player->up);
