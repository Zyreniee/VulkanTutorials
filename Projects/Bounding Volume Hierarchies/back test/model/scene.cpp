...
void Scene::buildBVH() {
	...
	glm::vec3 minCorner = glm::vec3(-100.0f, -100.0f, -20.0f);
	glm::vec3 maxCorner = glm::vec3(100.0f, 100.0f, 20.0f);
			...
			for (int i = 0; i < 2; ++i) {
				...
				for (int j = 0; j < 2; ++j) {
					...
					for (int k = 0; k < 2; ++k) {
						...
						Box3D* box = new Box3D(minCorner, maxCorner);
						node->children[i + 2 * j + 4 * k] = new Node(box);

						for (Sphere* sphere : node->spheres) {
							if (box->overlapsWith(sphere)) {
								node->children[i + 2 * j + 4 * k]->spheres.push_back(sphere);
							}
						}
						nodes.push(node->children[i + 2 * j + 4 * k]);
					}
				}
			}
