...
class GameBoard:
        ...
        for row in range(self.numRows):
            for column in range(self.numCols):
                ...
                if piece is not None:
                    piece.setMouseHover(False)
                    piece.setNeighbor(False)
                    piece.setSmashed(False)
                    piece.update()
        ...
        for row in range(self.numRows):
            for column in range(self.numCols):
                ...
                if firstPiece is not None:
                    ...
                    toSearch.put(self.pieces[row][column])
                    while not toSearch.empty():
                        piece = toSearch.get()
                        matches = self.expandSearch(piece)
                        searched.append(piece)
    ...
    def isStable(self):
        for row in range(self.numRows):
            for column in range(self.numCols):
                piece = self.pieces[row][column]
                if (piece is not None) and (not piece.isStable()):
                    return False
        return True
