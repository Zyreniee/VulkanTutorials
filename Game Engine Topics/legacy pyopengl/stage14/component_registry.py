...
class ComponentRegistry:
    ...
    def __init__(self):
        ...
        self.floor_command_count = 0
        self.wall_command_count = 0
        self.visited_mask = None

        self.ray_x = np.zeros(512, dtype=np.float32)
        self.ray_z = np.zeros(512, dtype=np.float32)
        self.ray_event = np.zeros(512, dtype=np.uint8)
    ...
    def set_map_size(self, height: int, width: int) -> None:
        ...
        self.visited_mask = np.zeros((width, height), dtype=np.uint8)
