...
class Player:
    ...
    def __init__(self, position):
        ...
        self.box = geometry.Box3D(0.35, 0.35, 0.9, position)
...
class Scene:
    ...
    def can_move(self, obj, velocity):
        ...
        temporary_box = geometry.Box3D(obj.box.l, obj.box.w, obj.box.h, obj.box.center + velocity)

        for block in self.blocks:
            block.color = np.array([1,0,0], dtype = np.float32)
            if block.box.overlaps_with(temporary_box):
                return False
    ...
    def move(self, obj, dt):
        ...
        if obj.box.center[2] <= 0.9:
            obj.box.center[2] = 0.9
            obj.on_ground = True
