...
class GameRenderer:
    ...
    def render(self, scene):
        ...
        glDisable(GL_CULL_FACE)
        glBindVertexArray(self.grid_debug_model.vao)
        for coord in geometry.grid.items:
            (x,y,z) = geometry.grid.grid_to_world(coord)
            modelTransform = pyrr.matrix44.create_from_translation(
                vec = np.array([x, y, z], dtype=np.float32), 
                dtype = np.float32
            )
            glUniformMatrix4fv(self.model_location["colored"], 1, GL_FALSE, modelTransform)
            glUniform3fv(self.color_location, 1, np.array([1, 1, 1], dtype=np.float32))
            glDrawArrays(GL_LINES, 0, self.grid_debug_model.vertex_count)
        glEnable(GL_CULL_FACE)
