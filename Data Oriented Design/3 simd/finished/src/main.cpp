...
public:
    ...
    World() {
        ...
        paddedCubeCount = cubeCount + (cubeCount & 7);
...
public:
    ...
    void run() {
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            /*
            for (size_t i = 0; i < world->cubeCount; ++i) {
                glm::mat4 pitch = glm::rotate(glm::mat4(1.0f), glm::radians(world->cubePitches[i]), {0, 1, 0});
                glm::mat4 yaw = glm::rotate(glm::mat4(1.0f), glm::radians(world->cubeYaws[i]), {0, 0, 1});
                glm::mat4 roll = glm::rotate(glm::mat4(1.0f), glm::radians(world->cubeRolls[i]), {1, 0, 0});
                glm::mat4 translate = glm::translate(glm::mat4(1.0f), world->cubePositions[i]);
