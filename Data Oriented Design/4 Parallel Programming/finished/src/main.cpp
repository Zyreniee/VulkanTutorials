...
public:
    ...
    void build_task_graph() {
        ...
        auto [pitch, yaw, roll] = taskGraph.emplace(
            [this]() {update_pitch(dt); },
            [this]() {update_yaw(dt); },
            [this]() {update_roll(dt); });
        
        auto writeMatrices = taskGraph.for_each_index(0, 100000, 1, [this](int i) {write_model_matrix(i); });

        writeMatrices.succeed(pitch, yaw, roll);
    }
